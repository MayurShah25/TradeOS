...
class SqlitePaperRunAuditRepository:
    ...
    def get_run(self, run_id: str) -> PaperTradingRun | None:
        ...
        row = self._connection.execute("SELECT * FROM paper_runs WHERE run_id = ?", (run_id,)).fetchone()
    ...
    def runs(
        ...
        *,
        ...
        rows = self._connection.execute(f"SELECT * FROM paper_runs{where} ORDER BY registration_order", parameters).fetchall()
    ...
    def append_audit_event(self, event: AuditEvent) -> None:
        ...
        next_sequence = self._connection.execute(
            "SELECT COALESCE(MAX(sequence) + 1, 0) FROM audit_events WHERE run_id = ?", (event.run_id,)
        ).fetchone()[0]
        ...
        try:
            self._connection.execute(
                ...
                (event.event_id, event.run_id, event.event_type.value, self._encode_datetime(event.occurred_at), event.sequence, json.dumps(event.payload, separators=(",", ":"))),
            )
            self._connection.commit()
    ...
    def audit_events(self, run_id: str) -> tuple[AuditEvent, ...]:
        ...
        rows = self._connection.execute(
            "SELECT event_id, run_id, event_type, occurred_at, sequence, payload FROM audit_events WHERE run_id = ? ORDER BY sequence",
            (run_id,),
        ).fetchall()
    ...
    def _validate_identity_unchanged(existing: PaperTradingRun, replacement: PaperTradingRun) -> None:
        fields = ("proposal_id", "risk_decision_id", "authorization_id", "account_id", "instrument_id", "configuration_hash", "started_at")
    ...
    def _run_values(run: PaperTradingRun) -> tuple[object, ...]:
        return (run.run_id, run.proposal_id, run.risk_decision_id, run.authorization_id, run.account_id, run.instrument_id, run.configuration_hash, SqlitePaperRunAuditRepository._encode_datetime(run.started_at), run.status.value, SqlitePaperRunAuditRepository._encode_datetime(run.completed_at))

    @staticmethod
    def _run_from_row(row: tuple[Any, ...]) -> PaperTradingRun:
        return PaperTradingRun(run_id=row[1], proposal_id=row[2], risk_decision_id=row[3], authorization_id=row[4], account_id=row[5], instrument_id=row[6], configuration_hash=row[7], started_at=SqlitePaperRunAuditRepository._decode_datetime(row[8]), status=PaperRunStatus(row[9]), completed_at=None if row[10] is None else SqlitePaperRunAuditRepository._decode_datetime(row[10]))

    @staticmethod
    def _event_from_row(row: tuple[Any, ...]) -> AuditEvent:
        event = AuditEvent(event_id=row[0], run_id=row[1], event_type=AuditEventType(row[2]), occurred_at=SqlitePaperRunAuditRepository._decode_datetime(row[3]), sequence=row[4], payload=tuple(tuple(item) for item in json.loads(row[5])))
        event.validate()
        return event
