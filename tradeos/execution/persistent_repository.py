...
class SqlitePaperRunAuditRepository:
        ...
        if existing is None:
            self._connection.execute(
                ...
                INSERT INTO paper_runs (
                    ...
                    account_id, instrument_id, configuration_hash, started_at,
                    status, completed_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
    ...
    def _validate_identity_unchanged(
        ...
        if any(getattr(existing, field) != getattr(replacement, field) for field in identity_fields):
    ...
    def _run_from_row(row: tuple[Any, ...]) -> PaperTradingRun:
        return PaperTradingRun(
            ...
            authorization_id=row[3],
            account_id=row[4],
            instrument_id=row[5],
            configuration_hash=row[6],
            started_at=SqlitePaperRunAuditRepository._decode_datetime(row[7]),
            status=PaperRunStatus(row[8]),
            completed_at=(
                None
                if row[9] is None
                else SqlitePaperRunAuditRepository._decode_datetime(row[9])
            ),
        )
