...
from tradeos.portfolio import AccountStateBuilder, OpenOrder, PortfolioStateBuilder, PositionLedger


def test_snapshot_integrates_account_positions_open_orders_and_timestamp() -> None:
    ledger = PositionLedger()
    ledger.apply_fill("AAPL", __import__("tradeos.execution", fromlist=["OrderSide"]).OrderSide.BUY, Decimal("10"), Decimal("100"))
...
def test_snapshot_rejects_non_utc_timestamp() -> None:
    ...
    timestamp = datetime(2026, 9, 2, 8, 0, tzinfo=__import__("datetime").timezone(__import__("datetime").timedelta(hours=1)))
...
def test_snapshot_rejects_naive_timestamp() -> None:
    ...
    with pytest.raises(ValueError, match="timezone-aware"):
        PortfolioStateBuilder.snapshot(ledger, timestamp=datetime(2026, 9, 2, 8, 0))
